...
router.put('/claim/:withdrawalId', (req, res) => {
    ...
    db.query('SELECT totalValue FROM withdrawal WHERE withdrawalId = ?', [withdrawalId], (err, withdrawalResult) => {
        if (err) {
        ...
        db.beginTransaction((err) => {
            ...
            db.query('UPDATE withdrawal SET userId = ? WHERE withdrawalId = ?',
                [userId, withdrawalId],
                (err, updateResult) => {
                    if (err) {
                        return db.rollback(() => {
